(function(exports) {
  ...
  cubism.context = function() {
    ...
    var step = 1e4; // ten seconds, in milliseconds
    var size = 1440; // four hours at ten seconds, in pixels
    var start0; var stop0; // the start and stop for the previous change event
    var start1; var stop1; // the start and stop for the next prepare event
    var serverDelay = 5e3;
    var clientDelay = 5e3;
    var event = d3.dispatch('prepare', 'beforechange', 'change', 'focus');
    var scale = context.scale = d3.time.scale().range([0, size]);
    var timeout;
    var focus;
 ...
 */
   ...
   */
    function find_ideal_librato_resolution(step) {
      ...
      var lowest_res = avail_rsts[avail_rsts.length]; // high and lowest available resolution from librato
  ...
  function cubism_graphiteParse(text) {
    var i = text.indexOf('|');
    var meta = text.substring(0, i);
    var c = meta.lastIndexOf(',');
    var b = meta.lastIndexOf(',', c - 1);
    var a = meta.lastIndexOf(',', b - 1);
    var start = meta.substring(a + 1, b) * 1000;
    var step = meta.substring(c + 1) * 1000;
    return text
      .substring(i + 1)
      .split(',')
  ...
  cubism_contextPrototype.gangliaWeb = function(config) {
    ...
    source.metric = function(metricInfo) {
      ...
      var isReport = metricInfo.isReport || false;
      var titleGenerator = metricInfo.titleGenerator ||
      ...
      var onChangeCallback = metricInfo.onChangeCallback;
  ...
  cubism_contextPrototype.metric = function(request, name) {
    var context = this;
    var metric = new cubism_metric(context);
    var id = '.metric-' + ++cubism_id;
    var start = -Infinity;
    var stop;
  ...
  function cubism_metricOperator(name, operate) {
    ...
    function cubism_metricOperator(left, right) {
      if (!(right instanceof cubism_metric)) { right = new cubism_metricConstant(left.context, right); } else if (left.context !== right.context) { throw new Error('mismatch context'); }
  ...
  cubism_contextPrototype.horizon = function() {
    ...
    var width = buffer.width = context.size();
    var height = buffer.height = 30;
    var scale = d3.scale.linear().interpolate(d3.interpolateRound);
    var metric = cubism_identity;
    var extent = null;
    var title = cubism_identity;
    ...
    function horizon(selection) {
      ...
      selection.each(function(d, i) {
        var that = this;
        var id = ++cubism_id;
        var metric_ = typeof metric === 'function' ? metric.call(that, d, i) : metric;
        var colors_ = typeof colors === 'function' ? colors.call(that, d, i) : colors;
        var extent_ = typeof extent === 'function' ? extent.call(that, d, i) : extent;
        var start = -Infinity;
        var step = context.step();
        var canvas = d3.select(that).select('canvas');
        var span = d3.select(that).select('.value');
        var max_;
        ...
        function change(start1, stop) {
          ...
          var i0 = 0; var max = Math.max(-extent[0], extent[1]);
          if (this === context) {
            if (max == max_) {
  ...
  cubism_contextPrototype.comparison = function() {
    ...
    var height = 120;
    var scale = d3.scale.linear().interpolate(d3.interpolateRound);
    var primary = function(d) { return d[0]; };
    var secondary = function(d) { return d[1]; };
    var extent = null;
    var title = cubism_identity;
    var formatPrimary = cubism_comparisonPrimaryFormat;
    var formatChange = cubism_comparisonChangeFormat;
    var colors = ['#9ecae1', '#225b84', '#a1d99b', '#22723a'];
    var strokeWidth = 1.5;
    ...
    function comparison(selection) {
      ...
      selection.each(function(d, i) {
        var that = this;
        var id = ++cubism_id;
        var primary_ = typeof primary === 'function' ? primary.call(that, d, i) : primary;
        var secondary_ = typeof secondary === 'function' ? secondary.call(that, d, i) : secondary;
        var extent_ = typeof extent === 'function' ? extent.call(that, d, i) : extent;
        var div = d3.select(that);
        var canvas = div.select('canvas');
        var spanPrimary = div.select('.value.primary');
        var spanChange = div.select('.value.change');
        var ready;
        ...
        function change(start, stop) {
          ...
          var secondaryExtent = secondary_.extent();
          var extent = extent_ == null ? primaryExtent : extent_;
        ...
        function focus(i) {
          ...
          var valueSecondary = secondary_.valueAt(i);
          var valueChange = (valuePrimary - valueSecondary) / valueSecondary;
  ...
  var cubism_comparisonChangeFormat = d3.format('+.0%');
  ...
  cubism_contextPrototype.axis = function() {
    ...
    var axis_ = d3.svg.axis().scale(scale);
  ...
  var cubism_axisFormatMinutes = d3.time.format('%I:%M %p');
  var cubism_axisFormatDays = d3.time.format('%B %d');
